...
public final class ZBS extends BlockSponge {
    ...
    public void updateTick(World map, int x, int y, int z, Random rnd) {
        ...
        if(map.getBlockMaterial(x+1, y, z) == Material.water) { map.setBlock(x+1,y,z, 19, 1, 0); map.scheduleBlockUpdate(x+1,y,z, 19, 5); }
        if(map.getBlockMaterial(x, y, z+1) == Material.water) { map.setBlock(x,y,z+1, 19, 1, 0); map.scheduleBlockUpdate(x,y,z+1, 19, 5); }
        if(map.getBlockMaterial(x-1, y, z) == Material.water) { map.setBlock(x-1,y,z, 19, 1, 0); map.scheduleBlockUpdate(x-1,y,z, 19, 5); }
        if(map.getBlockMaterial(x, y, z-1) == Material.water) { map.setBlock(x,y,z-1, 19, 1, 0); map.scheduleBlockUpdate(x,y,z-1, 19, 5); }
    }
