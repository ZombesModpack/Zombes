...
public final class ZBS extends BlockSponge {
    ...
    public void onBlockAdded(World map, int x, int y, int z) {
        ...
        if(water && map.getBlockMaterial(x, y+1, z) != Material.water) {
            map.setBlockAndMetadataWithNotify(x,y,z, 19, 1, 0);
    ...
    public void updateTick(World map, int x, int y, int z, Random rnd) {
        ...
        if(meta == 0) return;
        map.func_94575_c(x,y,z,0); // remove the block
        if(map.getBlockMaterial(x, y+1, z) == Material.water) return;

        if(map.getBlockMaterial(x+1, y, z) == Material.water) { map.setBlockAndMetadataWithNotify(x+1,y,z, 19, 1, 0); map.scheduleBlockUpdate(x+1,y,z, 19, 5); }
        if(map.getBlockMaterial(x, y, z+1) == Material.water) { map.setBlockAndMetadataWithNotify(x,y,z+1, 19, 1, 0); map.scheduleBlockUpdate(x,y,z+1, 19, 5); }
        if(map.getBlockMaterial(x-1, y, z) == Material.water) { map.setBlockAndMetadataWithNotify(x-1,y,z, 19, 1, 0); map.scheduleBlockUpdate(x-1,y,z, 19, 5); }
        if(map.getBlockMaterial(x, y, z-1) == Material.water) { map.setBlockAndMetadataWithNotify(x,y,z-1, 19, 1, 0); map.scheduleBlockUpdate(x,y,z-1, 19, 5); }
    }
