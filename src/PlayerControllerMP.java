...
public class PlayerControllerMP
{
    ...
    public static void clickBlockCreative(Minecraft par0Minecraft,
            PlayerControllerMP par1PlayerControllerMP, int par2, int par3,
            int par4, int par5)
    {
    ...
    public void setGameType(EnumGameType par1EnumGameType)
    {
        ...
        this.currentGameType
        .configurePlayerCapabilities(this.mc.thePlayer.capabilities);
    }
    ...
    public boolean onPlayerDestroyBlock(int par1, int par2, int par3, int par4)
    {
        if (this.currentGameType.isAdventure()
                && !this.mc.thePlayer.canCurrentToolHarvestBlock(par1, par2,
                        par3))
        {
            return false;
        }
    ...
    {
        // -ZMod-Dig-check-----------------------------------------------------
        if (!ZMod.digCheckReachDig(par1, par2, par3))
        {
            return;
        }
                        ...
                        par3))
        {
            if (this.currentGameType.isCreative())
            {
                this.netClientHandler.addToSendQueue(new Packet14BlockDig(0,
                        par1, par2, par3, par4));
                clickBlockCreative(this.mc, this, par1, par2, par3, par4);
                this.blockHitDelay = ZMod.blockHitDelay(); // -ZMod-clickmining
                // -ZMod-Dig-sync----------------------------------------------
                ZMod.onBlockDigged(par1, par2, par3, par4);
                // ------------------------------------------------------------
            }
            else if (!this.isHittingBlock
                    || !this.sameToolAndBlock(par1, par2, par3))
            {
                // -ZMod-Cheat-------------------------------------------------
                ZMod.beforeBlockDig();
                ...
                {
                    this.netClientHandler.addToSendQueue(new Packet14BlockDig(
                            1, this.currentBlockX, this.currentBlockY,
                            this.currentblockZ, par4));
                }
                ...
                {
                    Block.blocksList[var5].onBlockClicked(this.mc.theWorld,
                            par1, par2, par3, this.mc.thePlayer);
                }
                ...
                if (var5 > 0
                        ...
                        .getPlayerRelativeBlockHardness(
                                this.mc.thePlayer,
                                this.mc.thePlayer.worldObj, par1, par2,
                                par3) >= 1.0F)
                {
                ...
                else
                {
                    ...
                    this.mc.theWorld.destroyBlockInWorldPartially(
                            this.mc.thePlayer.entityId, this.currentBlockX,
                            this.currentBlockY, this.currentblockZ,
    ...
    public void resetBlockRemoving()
    {
        if (this.isHittingBlock)
        {
            this.netClientHandler.addToSendQueue(new Packet14BlockDig(1,
                    this.currentBlockX, this.currentBlockY, this.currentblockZ,
                    -1));
            // -ZMod-Cheat-----------------------------------------------------
            ZMod.afterBlockDig();
            // ----------------------------------------------------------------
    ...
    public void onPlayerDamageBlock(int par1, int par2, int par3, int par4)
    {
        ...
        {
            this.blockHitDelay = ZMod.blockHitDelay(); // -ZMod-clickmining
            this.netClientHandler.addToSendQueue(new Packet14BlockDig(0, par1,
                    par2, par3, par4));
            clickBlockCreative(this.mc, this, par1, par2, par3, par4);
            // -ZMod-Dig-sync--------------------------------------------------
            ZMod.onBlockDigged(par1, par2, par3, par4);
        ...
        else
        {
            if (this.sameToolAndBlock(par1, par2, par3))
            {
                ...
                this.curBlockDamageMP += var6.getPlayerRelativeBlockHardness(
                        this.mc.thePlayer, this.mc.thePlayer.worldObj, par1,
                        par2, par3);

                if (this.stepSoundTickCounter % 4.0F == 0.0F && var6 != null)
                {
                    this.mc.sndManager.playSound(var6.stepSound.getStepSound(),
    ...
    {
        return ZMod.actualPlayerReach(); // -ZMod-Dig-reach
    }
    ...
    private void syncCurrentPlayItem()
    {
        ...
        if (var1 != this.currentPlayerItem)
        {
            ...
            this.netClientHandler.addToSendQueue(new Packet16BlockItemSwitch(
    ...
    {
        // -ZMod-Dig-check-----------------------------------------------------
        if (!ZMod.digCheckReachUse(par4, par5, par6, par7))
        {
            return false;
        }
        ...
        ZMod.beforeBlockPlace();
        // --------------------------------------------------------------------
        this.netClientHandler.addToSendQueue(new Packet15Place(par4, par5,
                par6, par7, par1EntityPlayer.inventory.getCurrentItem(), var9,
                var10, var11));
        ...
        else if (this.currentGameType.isCreative())
        {
            ...
            boolean var15 = par3ItemStack.tryPlaceItemIntoWorld(
        ...
        {
            // -ZMod-Cheat-projection------------------------------------------
            boolean ret = par3ItemStack.tryPlaceItemIntoWorld(par1EntityPlayer,
    ...
    {
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new Packet15Place(-1, -1, -1, 255,
                par1EntityPlayer.inventory.getCurrentItem(), 0.0F, 0.0F, 0.0F));
        int var4 = par3ItemStack.stackSize;
        ItemStack var5 = par3ItemStack.useItemRightClick(par2World,
                par1EntityPlayer);
    ...
    {
        return new EntityClientPlayerMP(this.mc, par1World, this.mc.session,
    ...
    {
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new Packet7UseEntity(
    ...
    public ItemStack windowClick(int par1, int par2, int par3, int par4,
            EntityPlayer par5EntityPlayer)
    {
        short var6 = par5EntityPlayer.openContainer
                .getNextTransactionID(par5EntityPlayer.inventory);
        ItemStack var7 = par5EntityPlayer.openContainer.slotClick(par2, par3,
                par4, par5EntityPlayer);
        this.netClientHandler.addToSendQueue(new Packet102WindowClick(par1,
                par2, par3, par4, var7, var6));
        return var7;
    }
    ...
    {
        this.netClientHandler.addToSendQueue(new Packet108EnchantItem(par1,
                par2));
    }
    ...
    public void sendSlotPacket(ItemStack par1ItemStack, int par2)
    {
        if (this.currentGameType.isCreative())
        {
            this.netClientHandler.addToSendQueue(new Packet107CreativeSetSlot(
                    par2, par1ItemStack));
        }
    }
    ...
    {
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new Packet14BlockDig(5, 0, 0, 0,
                255));
